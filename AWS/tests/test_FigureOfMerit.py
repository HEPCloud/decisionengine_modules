...
def create_datablock():
    data_block = {}
    data_block['provisioner_resource_spot_prices'] =  pd.read_csv(os.path.join(DATA_DIR,'AWSSpotPriceWithSourceProxy_expected_acquire.csv'), float_precision='high')
    data_block['Performance_Data'] = pd.read_csv(os.path.join(DATA_DIR,'instance_performance_sample.csv')).drop_duplicates(subset=[ 'AvailabilityZone', 'InstanceType'], keep='last').reset_index(drop = True)
    data_block['Job_Limits'] = pd.read_csv(os.path.join(DATA_DIR,'job_limits.csv')).drop_duplicates(subset=['AvailabilityZone', 'InstanceType'], keep='last').reset_index(drop=True)
