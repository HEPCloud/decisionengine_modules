    ...
    def transform(self, data_block):
        ...
        for i, row in spot_price_data.iterrows():
            if perf_data.empty:
                ...
                r1 = perf_data.loc[(perf_data['AvailabilityZone'] == row['AvailabilityZone']) & (
                    perf_data['InstanceType'] == row['InstanceType'])]

            if occup_data.empty:
                r2 = occup_data
            else:
                r2 = occup_data[(occup_data['AvailabilityZone'] == row['AvailabilityZone']) & (
                    occup_data['InstanceType'] == row['InstanceType'])]

            if job_limits_data.empty:
                r3 = job_limits_data
            else:
                r3 = job_limits_data[(job_limits_data.AWSProfile == row['AccountName']) & (
                    job_limits_data['AvailabilityZone'] == row['AvailabilityZone']) & (job_limits_data['InstanceType'] == row['InstanceType'])]
