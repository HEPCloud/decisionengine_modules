...
class GlideFrontendElement:
    ...
    def create_glideclient_classads(
        ...
        ctkn = self.refresh_entry_token(glidein_site)  # TODO: Use a generator here
        if ctkn:
            payload_creds.append(
                create_credential(ctkn, purpose=CredentialPurpose.CALLBACK, cred_type=CredentialType.IDTOKEN)
            )
        ...
        for req_cred in self.request_credentials:
            ...
            # Add id for the credential file
            ...
            if isinstance(req_cred.credential, CredentialPair):
                glidein_params_to_encrypt[
                    req_cred.credential.private_credential.classad_attribute
                ] = req_cred.credential.private_credential.id
    ...
    def generate_glidein_requests_one(self, jobs_df, slots_df, entries, factory_globals, job_filter="ClusterId > 0"):
        ...
        for glideid in glideids_to_advertise:
            ...
            self.credential_plugin.generate_credentials(
                elementDescript=self.group_config,
                glidein_el=entry_info,
