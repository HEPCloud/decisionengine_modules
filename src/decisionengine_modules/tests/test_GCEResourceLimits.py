...
def test_produces():
    with mock.patch.object(configmanager, "ChannelConfigHandler"), mock.patch.object(dataspace, "DataSpace"):
        gce_resource_limits = GCEResourceLimits.GCEResourceLimits(CONFIG)
        assert gce_resource_limits._produces == _PRODUCES


def test_acquire():
    with mock.patch.object(configmanager, "ChannelConfigHandler"), mock.patch.object(
        dataspace, "DataSpace"
